...
export default function OrdersFilterBar({
  ...
  return (
    <search
      ...
      className="flex flex-col md:flex-row gap-4 items-center justify-between  dark:bg-stone-800 p-2 rounded-xl border border-gray-200 dark:border-stone-700"
    >
      <div
        role="group"
        aria-label="Filter by order status"
        className="flex flex-wrap gap-1 w-full md:w-auto"
      >
        {STATUSES.map((status) => (
          <button
            key={status}
